...
<div class="top position" style="height: 30px; border-bottom: 1px solid #ccc;">
	...
	<span style="vertical-align: middle; margin: 0 10px;">首页 > 用户管理 > 权限管理</span>
</div>
...
<div id="table" style="position: absolute; top: 50px; bottom: 60px; left: 10px; right: 10px; overflow: auto">
	...
	<table class="table table-striped table-hover" style="background: #eee">
		<tr style="background: #333;color: #eee; font-weight: bold">
			...
			<td style="width: 5em;">权限分类</td>
			<td style="width: 5em;">权限名称</td>
			<td style="width: 10em;">权限说明</td>
		...
		<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$l): $mod = ($i % 2 );++$i;?><tr>
			...
			<td>
				<a href="/SamCRM/index.php/Home/User/competence_detail?id=<?php echo ($l["id"]); ?>" class="edit"><img src="/SamCRM/Public/img/home/edit.png" style="margin-right: 5px;"></a>
				<a href="/SamCRM/index.php/Home/User/competence_del?id=<?php echo ($l["id"]); ?>" class="del" onclick="if(confirm('确认删除？')){return true}else{return false}"><img src="/SamCRM/Public/img/home/delete.png"></a>
			</td>
...
<script>
	...
	$("#check_del").click(function () {
		$("input[type=checkbox]").each(function (index,ele) {
			if($(ele).is(":checked")){
				$(ele).parent().parent().children().eq(7).children().eq(1).click();
			}
		});
	});
</script>
